...
int main(void) {
  ...
  FD_SET(socketPrincipal, &descriptoresLectura);
  maxDescriptor = socketPrincipal;
  ...
  for (;;) {
    ...
    if (FD_ISSET(socketPrincipal, &descriptoresTemporales)) {
      ...
      if (bytesRecibidos != 500) {
        std::cout << "ERROR: Tamaño de mensaje incorrecto\n";
        continue;
      }
      ...
      if (paquete[9] == 'N') {
        ...
        std::cout << "Cliente " << alias << " conectado desde " << claveCliente << "\n";
        continue;
      }
      ...
      if (estaCompleto) {
        ...
        procesarDatosCliente(std::move(partes), alias, socketPrincipal, tipo);
      }
    }
  }
