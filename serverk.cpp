...
std::atomic<long long> secuenciaRDT(0); // Nuevo: Contador de secuencia para RDT
...
// Función ajustada para devolver un string de 5 dígitos (para el nuevo protocolo RDT).
std::string calcularSumaVerificacion(const char *bufferDatos, long tamanoDatos) {
    ...
    std::stringstream ss;
    ss << std::setw(5) << std::setfill('0') << valorSumaVerificacion;
    return ss.str();
}

void enviarKeepAlive(int descriptorSock, const sockaddr_in &puntoExtremoServidor) {
    char paqueteRDT[500];
    std::memset(paqueteRDT, 0, 500);
    
    // Contenido del paquete K (Data)
    std::string data = "OK"; 
    long long numSecuencia = ++secuenciaRDT;
    
    // Construcción del paquete RDT: TAG(1) + SEQ(10) + CHKSUM(5) + DATA
    // TAG (1 byte)
    paqueteRDT[0] = 'K'; 
    
    // NUMERO DE SECUENCIA (10 bytes)
    std::sprintf(paqueteRDT + 1, "%010lld", numSecuencia); 
    
    // CHECKSUM (5 bytes)
    std::string checksum = calcularSumaVerificacion(data.c_str(), data.length());
    std::memcpy(paqueteRDT + 1 + 10, checksum.c_str(), 5);
    
    // DATA
    std::memcpy(paqueteRDT + 1 + 10 + 5, data.c_str(), data.length());

    if (sendto(descriptorSock, paqueteRDT, 500, 0,
               (const sockaddr *)&puntoExtremoServidor, sizeof(puntoExtremoServidor)) == -1)
        perror("sendto KeepAlive");
}

void keepAliveLoop(int descriptorSock, sockaddr_in direccionServidor) {
    while (true) {
        std::this_thread::sleep_for(std::chrono::seconds(15));
        enviarKeepAlive(descriptorSock, direccionServidor);
        {
             std::lock_guard<std::mutex> bloqueoConsola(exclusividadConsola);
             std::cout << "\n[Keep Alive enviado: OK]\n";
             std::cout.flush();
        }
    }
}
...
void recibirDatosServidor(int descriptorSock, sockaddr_in puntoExtremoServidor) {
    ...
    while (true) {
        ...
        // El servidor puede enviar un 'K' (Keep Alive)
        if (paqueteRecibido[0] == 'K') {
            printf("[Keep Alive] Recibido: %s\n", paqueteRecibido.c_str());
            continue;
        }

        // Lógica de reensamblaje para el protocolo de mensajes existente (no RDT)
...
void manejarMensajesEntrantes(std::vector<std::string> fragmentosMensaje, std::string aliasActual, int descriptorSock, char tipoMensaje) {
    ...
    if (tipoMensaje == 'l') {
        ...
        std::string hashLocalStr = calcularSumaVerificacion(contenidoArchivo.c_str(), longitudFinalArchivo);
        int hashLocal = std::stoi(hashLocalStr);
        ...
        std::cout << "\nERROR " << codigoError << ": " << mensajeError << "\n";
...
int main(void) {
    ...
    // El paquete de registro ('N') sigue usando el formato antiguo
    ...
    // **NUEVO: Iniciar el hilo Keep Alive**
    std::thread(keepAliveLoop, socketUDP, direccionServidor).detach();
    ...
    while (!salirCliente) {
        ...
        if (consultaVisualizacion) {
            ...
            std::string cadenaHash = calcularSumaVerificacion(bytesArchivo.data(), tamanoArchivoTotal);
