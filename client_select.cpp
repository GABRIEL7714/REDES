...
void recibirDatosServidor(int descriptorSock, sockaddr_in puntoExtremoServidor) {
    ...
    int maxDescriptor = descriptorSock;

    FD_ZERO(&descriptoresLectura);
    FD_SET(descriptorSock, &descriptoresLectura);
    ...
    while (true) {
        fd_set descriptoresTemporales = descriptoresLectura;
        
        if (select(maxDescriptor + 1, &descriptoresTemporales, NULL, NULL, NULL) == -1) {
            perror("select");
            continue;
        }

        if (FD_ISSET(descriptorSock, &descriptoresTemporales)) {
...
int main(void) {
    ...
    fd_set descriptoresLectura, descriptoresTemporales;
    int maxDescriptor;
    ...
    // Configurar select
    FD_ZERO(&descriptoresLectura);
    FD_SET(socketUDP, &descriptoresLectura);
    FD_SET(STDIN_FILENO, &descriptoresLectura);
    maxDescriptor = (socketUDP > STDIN_FILENO) ? socketUDP : STDIN_FILENO;
    ...
    while (!salirCliente) {
        descriptoresTemporales = descriptoresLectura;
        ...
        if (FD_ISSET(STDIN_FILENO, &descriptoresTemporales)) {
            {
                std::lock_guard<std::mutex> bloqueoConsola(exclusividadConsola);
                if (!enPartidaActiva) {
                    std::cout << "\n=== MENÚ PRINCIPAL ===\n"
                ...
                if (consultaVisualizacion) {
                    std::cout << "¿Quieres ver la partida? (y/n)\n";
                }
                std::cout << "> ";
                std::cout.flush();
            }
